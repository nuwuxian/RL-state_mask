...
class Losses(collections.namedtuple("Losses", "policy value entropy lasso")):
  """Losses from a training step."""

  @property
  def total(self):
    return self.policy + C_1 * self.value - C_2 * self.entropy + lambda_1 * self.lasso

  def __str__(self):
    return ("Losses(total: {:.3f}, policy: {:.3f}, value: {:.3f}, \
            l2: {:.3f}, lasso: {:.3f})").format(self.total, self.policy, self.value, self.entropy, self.lasso)
  ...
  def __truediv__(self, n):
    return Losses(self.policy / n, self.value / n, self.entropy / n, self.lasso / n)
...
def _play_game(logger, game_num, game, bots, mask_net, temperature, temperature_drop):
  ...
  logger.print("Mask ratio: {}".format(len(trajectory.mask_pos)/trajectory.eps_len*2))
  return trajectory
  ...
  def learn(step):
    ...
    for epoch in range(config.n_epochs):
      for _ in range(len(replay_buffer) // config.train_batch_size):
        ...
        num_nomasks = torch.sum(onehot_action[:, 1])
...
def test(*, game, config, logger):
  ...
  if config.test_masknet:
     ...
     #model.load_checkpoint("/home/zxc5262/models_againstmcts/checkpoints/model_7000.pth")
  logger.print("Initializing bots")
  base_model = load_pretrain(config.az_path)
  az_evaluator = evaluator_lib.AlphaZeroEvaluator(game, base_model)
  random_evaluator = mcts.RandomRolloutEvaluator()
  ...
  if config.test_masknet:
    ...
    for i in important_thresholds:
      ...
      for game_num in range(500):
        ...
        trajectory = replay(logger, game_num, game, bots, path, critical_step_start, critical_step_end, \
      ...
      for game_num in range(500):
        ...
        trajectory = replay(logger, game_num, game, bots, path, non_critical_step_start, non_critical_step_end, \
                            random_replace=False, orig_traj_len=orig_traj_len, temperature=1,
                              temperature_drop=0)
        replay_results.append(trajectory.returns[az_player])

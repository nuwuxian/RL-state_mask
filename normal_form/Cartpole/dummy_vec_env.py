...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns: List[Callable[[], gym.Env]]):
        ...
        # training pool 
        losing_game_file = 'losing_game.out'
        ...
        self.critical_steps_starts = np.loadtxt('./retrain/critical_steps_starts.out')
    ...
    def reset(self) -> VecEnvObs:
        # replay and reset
        for env_idx in range(self.num_envs):
            idx = int(random.choice(self.idxs_list))
            obs = self.env[env_idx].reset()
            self.env[env_idx].seed(idx)
            action_path = './retrain_data/act_seq_' + str(idx) + '.out'
            recorded_actions = np.loadtxt(action_path)
            cnt = 0
            while cnt < self.critical_steps_starts[idx]:
                obs, _, _, _ = self.env[env_idx].step(recorded_actions[cnt])
                cnt += 1
            self._save_obs(env_idx, obs)

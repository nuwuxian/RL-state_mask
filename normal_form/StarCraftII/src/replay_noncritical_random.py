...
def main(argv):
    path = 'results/'
    non_critical_steps_starts = np.loadtxt(path + str(FLAGS.threshold)+ "non_critical_steps_starts.out")
    non_critical_steps_ends = np.loadtxt(path + str(FLAGS.threshold)+ "non_critical_steps_ends.out")
    ...
    for game_num in range(FLAGS.num_episodes):
        ...
        replay_result = replay_traj(env, mask_agent, agent, game_num, non_critical_step_start, non_critical_step_end, orig_traj_len, act_buf, random_replace=True)
        replay_reward.append(replay_result)
        sum_return += replay_result

    np.savetxt(path + str(FLAGS.threshold)+ "noncritical_random_reward.out", replay_reward)

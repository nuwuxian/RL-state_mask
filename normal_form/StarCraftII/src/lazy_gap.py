...
def select_steps(path, critical, import_thrd):
  ...
  for i_episode in range(FLAGS.num_episodes):
    lazy_gaps_path = path + "lazy_gaps_" + str(i_episode) + ".out" 
    lazy_gaps = np.loadtxt(lazy_gaps_path)
    ...
    k = max(int(len(lazy_gaps) * import_thrd),1)
    ...
    try:
        ...
        while not done:
            ...
            for action in legal_action:
                ...
                (next_state_copy_0, next_state_copy_1), reward_copy, _ , _ = env_copy.step([action, action_1])
                _, next_value_copy, _, _, _ = agent._model.step(transform_tuple(next_state_copy_0, lambda x: np.expand_dims(x, 0)), True)
                Q_value = reward_copy + FLAGS.discount_gamma * next_value_copy[0]
...
def main(argv):
    ...
    for i in range(len(important_thresholds)):
        ...
        for game_num in range(FLAGS.num_episodes):
            ...
            act_buf = np.load(path + "act_seq_" + str(game_num) + ".out")
            oppo_act_buf = np.load(path + "oppo_act_seq_" + str(game_num) + ".out")

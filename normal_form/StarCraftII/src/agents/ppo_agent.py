...
class PPOActor(object):
  ...
  def __init__(self, env, policy, unroll_length, gamma, lam, queue_size=1,
    ...
    self._base_model = Model(policy=MlpPolicy,
                        scope_name="base_model",
                        ob_space=env.observation_space,
                        ac_space=env.action_space,
                        nbatch_act=1,
                        nbatch_train=unroll_length,
    ...
    self._base_state = self._base_model.initial_state
    self._done = False
    self._cum_reward = 0

    self._base_model.load(model_path)
  ...
  def _nstep_rollout(self):
    ...
    for _ in range(self._unroll_length):
      ...
      base_action, _, self._base_state, _, mask = self._base_model.step(
          transform_tuple(self._obs, lambda x: np.expand_dims(x, 0)),
          self._base_state,
          np.expand_dims(self._done, 0))
      
      ###mask operation####
      self._model.mask_record+=1
      if self._model.mask_record>=200:
        self._model.mask_record=0
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~The mask ratio is ", self._model.mask_ratio / 200)
        self._model.mask_ratio=0
      if action[0]==0:
        real_action = base_action[0]
      elif action[0]==1:
        legal_action = np.where(mask[0]==1)[0]
        real_action = np.random.choice(legal_action)
        # real_action = 0
        self._model.mask_ratio+=1
      ###mask operation####
      ...
      self._obs, reward, self._done, info = self._env.step(real_action)
      ...
      if self._done:
        ...
        self._cum_reward = 0
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~The reward is ", self._cum_reward)
...
class PPOSelfplayActor(object):
  ...
  def __init__(self, env, policy, mask_policy, unroll_length, gamma, lam, model_cache_size,
               model_cache_prob, queue_size=1, prob_latest_opponent=0.0,
               init_opponent_pool_filelist=None, freeze_opponent_pool=False,
               enable_push=True, learner_ip="localhost", port_A="5700",
               port_B="5701", model_path=None, mask_path=None):
    assert isinstance(env.action_space, spaces.Discrete)
    self._env = env
    self._unroll_length = unroll_length
    self._lam = lam
    ...
    self._model = Model(policy=mask_policy,
                        scope_name="model",
                        ob_space=env.observation_space,
                        ac_space=env.action_space,
                        nbatch_act=1,
                        ...
                        vf_coef=0.5,
                        max_grad_norm=0.5)
    self._base_model = Model(policy=policy,
                        scope_name="base_model",
                        ob_space=env.observation_space,
                        ac_space=env.action_space,
                        nbatch_act=1,
    ...
    if mask_path is not None:
      self._model.load(mask_path)
    self._oppo_model.load(model_path)
    self._base_model.load(model_path)
  ...
  def _nstep_rollout(self):
    ...
    for _ in range(self._unroll_length):
      ...
      oppo_action, _, self._oppo_state, _ , _ = self._oppo_model.step(
      ...
      (self._obs, self._oppo_obs), reward, self._done, info = self._env.step(
        [real_action, oppo_action[0]])
      # print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~The action is ", real_action, oppo_action[0])
      self._cum_reward += reward
      if self._done:

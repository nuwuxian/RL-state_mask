...
def main(argv):
    ...
    for game_num in range(FLAGS.num_episodes):
        ...
        replay_result = replay_traj(env, mask_agent, agent, game_num, critical_step_start, critical_step_end, orig_traj_len, act_buf, random_replace=True)
        replay_reward.append(replay_result)
        sum_return += replay_result

    np.savetxt(path + str(FLAGS.threshold) + "critical_random_reward.out", replay_reward)

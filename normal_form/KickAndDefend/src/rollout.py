...
def rollout(victim_agent, adv_agent, mask_agent, env, num_traj, max_ep_len=1e3, save_path=None):
    ...
    for i in range(num_traj):
        ...
        while episode_length < max_ep_len:
            ...
            for id, obs in enumerate(observation):
                ...
                #    clipped_actions = np.clip(act, env.action_space[0].low, env.action_space[0].high)
                    ...
                    act, v = adv_agent.act(observation=obs)
                    value_seq.append(v['vpred'])
        ...
        if 'winner' in infos[0]:
            eta_sum += 1 * 0.99 ** (episode_length - 1)
        ...
        mask_pos_filename = "./recording/value_seq_" + str(i) + ".out" 
        np.savetxt(mask_pos_filename, value_seq)

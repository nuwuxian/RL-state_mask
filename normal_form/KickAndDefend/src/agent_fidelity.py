...
def load_zoo_agent(env_name, ob_space, action_space, tag=1, version=3, scope=""):
    ...
    if tag==1:
        env_path = '/data/jiahao/mujoco/fidelity_kicker/multiagent-competition/agent-zoo/kick-and-defend/kicker/agent1_parameters-v1.pkl'
    if tag==2: 
        env_path = '/data/jiahao/mujoco/fidelity_kicker/multiagent-competition/agent-zoo/kick-and-defend/defender/agent2_parameters-v1.pkl'
...
class ZooAgent(object):
    ...
    def act(self, observation, reward=None, done=None):
        return self.agent.act(stochastic=False, observation=observation)
...
def load_adv_agent(ob_space, action_space, n_envs, adv_model_path, adv_ismlp=True, name=None):
    ...
    test = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
    adv_agent_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=name+'/model')
...
class AdvAgent(object):
    def __init__(self, ob_space, action_space, n_envs, adv_model_path, adv_ismlp, adv_obs_normpath=None, name=None):
        self.agent = load_adv_agent(ob_space, action_space, n_envs, adv_model_path, adv_ismlp, name)
        self.state = None
    ...
    def act(self, observation, reward=None, done=None):
        ...
        action, _, self.state, neglogp = self.agent.step(obs=observation, state=self.state, mask=done, deterministic=True)
...
def make_adv_agent(ob_space, action_space, n_envs, adv_model_path, adv_ismlp, adv_obs_normpath=None, name=None):
    return AdvAgent(ob_space, action_space, n_envs, adv_model_path, adv_ismlp, adv_obs_normpath, name)

...
from zoo_utils import load_from_file, load_from_model, setFromFlat, LSTMPolicy, MlpPolicyValue
from stable_baselines.a2c.utils import total_episode_reward_logger
from value import MlpLstmValue, MlpValue
import pdb
from agent import AdvAgent, ZooAgent
...
class MyPPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.2,
        ...
        self.vf_loss = None
        self.pg_loss = None
        self.lasso_loss = None
        self.fuss_lasso_loss = None
        self.approxkl = None
        self.clipfrac = None
        self.params = None
        self._train = None
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                if self.retrain_vicitim and not self.use_baseline_policy:
                    ...
                    # mask_action_space = spaces.Box(np.array([0,0]), np.array([1,1]))
                    mask_action_space = spaces.Discrete(2)
                    # action model mask: (n_envs, 1), state * (1-mask), mask=1 means reset state.
                    act_model = self.policy(self.sess, self.observation_space, mask_action_space, self.n_envs, 1,
                    ...
                    with tf.variable_scope("train_model", reuse=True,
                        ...
                        # custom_getter = tf_util.outer_scope_getter("train_model")
                        train_model = self.policy(self.sess, self.observation_space, mask_action_space,
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.mask_ph = tf.placeholder(tf.float32, [None], name="mask_ph")
                    self.mask_ratio = tf.placeholder(tf.float32, [None], name="mask_ratio")
                    ...
                    self.mask_ph = tf.cast(self.action_ph, tf.float32)
                    self.lasso_loss = tf.reduce_sum(self.mask_ph)
                    self.fused_lasso =  tf.reduce_sum(tf.square(self.mask_ph) - tf.roll(self.mask_ph, 1, 0))
                    # final ppo loss
                    loss = self.pg_loss - self.entropy * self.ent_coef + self.vf_loss * self.vf_coef +  self.lasso_coef * self.lasso_loss + self.fuss_lasso_coef * self.fused_lasso
                    loss = tf.cond(self.lambda_ph > 1.0, lambda: -1.0 * loss, lambda: loss)
    ...
    def _train_step(self, learning_rate, cliprange, coef_opp, coef_adv, coef_abs, obs, returns, masks, actions, values, neglogpacs,
        ...
        normalize_advs = (advs - advs.mean()) / (advs.std() + 1e-8)

        opp_advs = opp_returns - opp_values
        normalize_opp_advs = (opp_advs - opp_advs.mean()) / (opp_advs.std() + 1e-8)

        abs_advs = abs_returns - abs_values
        normalize_abs_advs = (abs_advs - abs_advs.mean()) / (abs_advs.std() + 1e-8)
        ...
        if self.retrain_vicitim and not self.use_baseline_policy:
            ...
            td_map = {self.train_model.obs_ph: obs, self.action_ph: actions, self.advs_ph: normalize_advs,
                      self.rewards_ph: returns,
                      self.learning_rate_ph: learning_rate, self.clip_range_ph: cliprange,
                      self.old_neglog_pac_ph: neglogpacs, self.old_vpred_ph: values,
                      self.opp_advs_ph: normalize_opp_advs,
                      self.opp_rewards_ph: opp_returns, self.old_opp_vpred_ph: opp_values,
                      self.abs_advs_ph: normalize_abs_advs,
                      ...
                      self.origin_advs_ph: advs, self.origin_opp_advs_ph: opp_advs, self.origin_abs_advs_ph: abs_advs,
                      self.lambda_ph: self.lambda_penalty
                      }
            ...
            td_map = {self.train_model.obs_ph: obs, self.action_ph: actions, self.advs_ph: normalize_advs, self.rewards_ph: returns,
                      self.learning_rate_ph: learning_rate, self.clip_range_ph: cliprange,
                      self.old_neglog_pac_ph: neglogpacs, self.old_vpred_ph: values,
                      self.opp_advs_ph: normalize_opp_advs,
                      self.opp_rewards_ph: opp_returns, self.old_opp_vpred_ph: opp_values,
        ...
        # else:
        est, policy_loss, value_loss, policy_entropy, approxkl, clipfrac, _, \
        _, opp_vf_loss, opp_pg_loss, adv_pg_loss, abs_pg_loss = self.sess.run(
            [self.est, self.pg_loss, self.vf_loss, self.entropy, self.approxkl, self.clipfrac, self._train,
             self._train_value, self.opp_vf_loss, self.opp_pg_loss, self.adv_pg_loss, self.abs_pg_loss], td_map)

        return est, policy_loss, value_loss, policy_entropy, approxkl, clipfrac, \
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            runner = Runner(env=self.env, model=self, n_steps=self.n_steps, gamma=self.gamma,
                            ...
                            is_mlp=self.is_mlp, use_victim_ob=use_victim_ob, adv_agent=self.adv_agent)
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, n_steps, gamma, lam, v_model, v1_model, is_mlp, use_victim_ob, adv_agent):
        """
    ...
    def run(self):
        ...
        for _ in range(self.n_steps):
            ...
            mb_actions.append(mask_actions)

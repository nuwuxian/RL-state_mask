...
for i in range(num_traj):
    ...
    while not done and episode_length < max_ep_len:
        ...
        if episode_length < critical_steps_starts[i]:
            clip_adv_action = np.clip(adv_recorded_actions[episode_length], env.action_space.spaces[0].low, env.action_space.spaces[0].high)
            clip_vic_action = np.clip(vic_recorded_actions[episode_length], env.action_space.spaces[0].low, env.action_space.spaces[0].high)

...
@watcher
def test(*, game, config, test_idx, q, logger):
  ...
  if config.test_masknet:
    ...
    for game_num in range(10):
      ...
      ret_0 = fid_play(logger, game_num, game, bots, None, 
    ...
    # return a tuple
    q.push((base_win, attack_win, attack_ratio))

def alpha_zero_attack(config: Config):
  ...
  for _ in range(int(FLAGS.num_tests/10)):
    p = ctx.Process(
            target=test,

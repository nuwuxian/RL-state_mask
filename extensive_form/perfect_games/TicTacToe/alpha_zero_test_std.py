...
flags.DEFINE_string("masknet_path", "/home/zxc5262/models_ttt/checkpoints/model_-1.pth", "Where to save masknet checkpoints.")
flags.DEFINE_integer("num_tests", 1500, "total test rounds")
FLAGS = flags.FLAGS
...
def test(*, game, config, test_idx, logger):
  ...
  if config.test_masknet:
    ...
    for i in range(len(important_thresholds)):
      ...
      logger.print(np.mean(np.subtract(critical_steps_ends, critical_steps_starts)))
      ...
      for game_num in range(500):
        ...
        critical_ratios.append(2*(critical_step_end - critical_step_start + 1)/42)
...
def alpha_zero_test(config: Config):
  ...
  path = config.path
  test_processes = [multiprocessing.Process(target=test, kwargs={"game": game, "config": config, "test_idx": i, "num": i}) for i in range(int(FLAGS.num_tests/500))]
  ...
  for test_idx in range((int(FLAGS.num_tests/500))):
  ...
  critical_perform = []
  critical_perform_std = []
  rand_critical_perform = []
  rand_critical_perform_std = []
  noncritical_perform = []
  noncritical_perform_std = []
  rand_noncritical_perform = []
  rand_noncritical_perform_std = []
  for i in range(4):
    critical_perform.append(np.mean(critical_performs[i]))
    critical_perform_std.append(np.std(critical_performs[i]))
    rand_critical_perform.append(np.mean(rand_critical_performs[i]))
    rand_critical_perform_std.append(np.std(critical_performs[i]))
    noncritical_perform.append(np.mean(noncritical_performs[i]))
    noncritical_perform_std.append(np.std(noncritical_performs[i]))
    rand_noncritical_perform.append(np.mean(rand_noncritical_performs[i]))
    rand_noncritical_perform_std.append(np.std(rand_noncritical_performs[i]))


  print("Baseline performance:", baseline_reward)
  print("Masknet performance: ", avg_reward)
  print("Replay (important) mean: ", critical_perform)
  print("STD: ", critical_perform_std)
  print("Replay (rand important): ", rand_critical_perform)
  print("STD: ", rand_critical_perform_std)
  print("Replay (nonimportant): ", noncritical_perform)
  print("STD: ", noncritical_perform_std)
  print("Replay (rand nonimportant): ", rand_noncritical_perform)
  print("STD: ", rand_noncritical_perform_std)

...
flags.DEFINE_string("path", "/home/zck7060/breakthrough/retrain/retrain_model/ours/", "Where to save checkpoints.")
flags.DEFINE_integer("checkpoint_freq", 1, "Save a checkpoint every N steps.")
flags.DEFINE_integer("actors", 5, "How many actors to run.")
flags.DEFINE_integer("evaluators", 1, "How many evaluators to run.")
flags.DEFINE_integer("evaluation_window", 20,
                     "How many games to average results over.")
flags.DEFINE_bool("retrain", False, "retrain or not")
...
def main(unused_argv):
  ...
  config = retrain_mcts.Config(
      ...
      output_size=None,
      quiet=FLAGS.quiet,
      #az_path="/home/zck7060/breakthrough/baseline/models/checkpoint--1"
      az_path = "retrain_model/ours/checkpoint-"
  )
  if FLAGS.retrain:
     retrain_mcts.alpha_zero_retrain(config)
  else:
     eval.alpha_zero_test(config)
